    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config._attn_implementation == "flash_attention_2":
            ...
            attn_output, k = attention_interface(
                self,
                query_states,
                key_states,
                value_states,
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, enable_kdv=False, **kwargs) -> torch.Tensor:
        """
        Args:
        ...
        for layer_num, blk in enumerate(self.blocks):
            ...
            if layer_num == len_blocks - 1 and enable_kdv:
                hidden_states, attn_key = blk(
                    hidden_states,
        ...
        if enable_kdv:
            with torch.no_grad():
                ...
                attn_key = attn_key[:, reverse_indices, :] # keeps per-head keys
            return hidden_states, attn_key
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def get_video_features(
        self, pixel_values_videos: torch.FloatTensor, video_grid_thw: Optional[torch.LongTensor] = None, enable_kdv=False
    ):
        """
        ...
        video_embeds, attn_key = self.visual(pixel_values_videos, grid_thw=video_grid_thw, enable_kdv=enable_kdv)
        split_sizes = (video_grid_thw.prod(-1) // self.visual.spatial_merge_size**2).tolist()
        video_embeds = torch.split(video_embeds, split_sizes)
        return video_embeds, attn_key

    def get_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None, enable_kdv=False):
        """
        ...
        image_embeds, attn_key = self.visual(pixel_values, grid_thw=image_grid_thw, enable_kdv=enable_kdv)

        split_sizes = (image_grid_thw.prod(-1) // self.visual.spatial_merge_size**2).tolist()
        image_embeds = torch.split(image_embeds, split_sizes)
        return image_embeds, attn_key
    ...
    def forward(
        ...
        enable_kdv: Optional[bool] = None,
        kdv_ratio: Optional[float] = None,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            image_embeds, attn_key = self.get_image_features(pixel_values, image_grid_thw=image_grid_thw, enable_kdv=enable_kdv)
        ...
        if pixel_values_videos is not None:
            select_pixel = True
            video_embeds, attn_key = self.get_video_features(pixel_values_videos, video_grid_thw=video_grid_thw, enable_kdv=enable_kdv)
        ...
        if select_pixel and (enable_kdv or enable_policy):
            ...
            if enable_policy:
                ...
                n_keep = int(num_tokens * (1.0 - kdv_ratio))
